...
查阅日期：2026-09-09。参考的是工程结构、初始化方式和配置模式，本项目未整体复制第三方业务代码。
...
后端锁定 Java 21、Spring Boot 3.5.11、MyBatis-Plus 3.5.12、springdoc 2.8.16。依据 [Spring Boot 3.5 系统要求](https://docs.spring.io/spring-boot/3.5/system-requirements.html)、[MyBatis-Plus 官方安装方式](https://baomidou.com/en/getting-started/install/)及 [springdoc 兼容矩阵](https://springdoc.org/#what-is-the-compatibility-matrix-of-springdoc-openapi-with-spring-boot)选择同一兼容系列，并通过实际依赖解析和测试确认。
...
若后续直接引入第三方源码、模板资源或图标，应按对应仓库许可证保留版权声明。本轮没有 vendoring 整个外部项目，也没有把第三方仓库说明当成本项目已实现能力。
